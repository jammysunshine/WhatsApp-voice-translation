...
class WhatsAppService {
  ...
  async getMediaUrl(mediaId) {
    try {
      ...
      // For media operations, we need to call the direct media endpoint
      // The base URL includes the phone number ID, but media endpoints don't need it
      // So we create a new client for media operations
      const mediaClient = axios.create({
        baseURL: this.apiUrl, // Just the base API URL, no phone number ID
        headers: {
          'Authorization': `Bearer ${this.token}`,
          'Content-Type': 'application/json'
        }
